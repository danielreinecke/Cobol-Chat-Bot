"""
Generate training data for COBOL tutor chatbot
Generates questions covering COBOL structure, debugging, 
mainframe concepts, and functions for model training
"""
...
def generate_jcl_qa():
    ...
    return [
        {"text": f"Q: What is JCL and what is it used for?\n A: Job Control Language (JCL) is a scripting language used on IBM mainframe operating systems (primarily z/OS) to instruct the system on how to run batch jobs, manage datasets, and allocate resources. It acts as the bridge between application programs (like COBOL) and the operating system, defining what program to execute, inputs/outputs required, and the handling of results."},
        {"text": f"Q: Generate me a small sample JCL job to run a COBOL program.\n A: Sure! Here is a simple example of a JCL job that runs a COBOL program {load_example_file('example_easy_JCL.txt')}\n\nWhen submitted for execution:\nMYJOB\nIs a jobname the system associates with this workload.\nMYSORT\nIs the stepname, which instructs the system to execute the SORT program.\nSORTIN\nOn the DD statement, SORTIN is the ddname. The SORTIN ddname is coded in the SORT program as a program input. The data set name (DSN) on this DD statement is ZPROF.AREA.CODES. The data set can be shared (DISP=SHR) with other system processes. The data content of ZPROF.AREA.CODES is SORT program input.\nSORTOUT\nThis ddname is the SORT program output.\nSYSOUT\nSYSOUT=* specifies to send system output messages to the Job Entry Subsystem (JES) print output area. It is possible to send the output to a data set.\nSYSIN\nDD * is another input statement. It specifies that what follows is data or control statements. In this case, it is the sort instruction telling the SORT program which fields of the SORTIN data records are to be sorted."},
        {"text": f"Q: What is a JOB statement?\n A: "},
        {"text": f"Q: What is the EXEC statement?\n A: "},
        {"text": f"Q: What is a DD statement?\n A: "},
        {"text": f"Q: How do I specify dataset location and allocation in JCL?\n A: "},
        {"text": f"Q: What are JCL procedures and how do I use them?\n A: "},
        {"text": f"Q: How do I use conditional execution (COND) in JCL?\n A: "},
        {"text": f"Q: What is the DISP parameter and what are its options?\n A: "},
        {"text": f"Q: How do I pass parameters to programs using JCL?\n A: "},
        {"text": f"Q: What is the difference between IN-STREAM and CATALOGED procedures?\n A: "},
        {"text": f"Q: How do I handle job failure and return codes in JCL?\n A: "},
        {"text": f"Q: Is there a way to make Job A wait for Job B to finish before continuing?\n A: "},
